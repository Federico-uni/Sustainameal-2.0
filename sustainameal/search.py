...
def find_nearest_recipes_by_tags_and_id(recipe_id, recipes_df, nutrient_vectors_df, tags_to_match, match_all_tags=True, n=10, distance_metric='cosine'):
    ...
    if match_all_tags:
        filtered_df = recipes_df[recipes_df['tags'].apply(lambda tags: set(tags_to_match).issubset(set(ast.literal_eval(tags))))]
    else:
        #filtered_df = recipes_df[recipes_df['tags'].apply(lambda tags: bool(set(tags_to_match) & set(ast.literal_eval(tags))))] Optional to test
