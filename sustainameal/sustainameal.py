import sys
sys.path.append('/kaggle/input/final-sustainameal-package/sustainameal')
...
from sustainameal.preprocessing import remove_duplicate_titles, remove_recipes_without_tags, invert_sustanability_score
from sustainameal.search import find_similar_by_title, find_nearest_recipes_by_tags_and_id, \
    find_nearest_recipes_by_nutrients_and_tags
from sustainameal.utils import calculate_centroids_and_find_common_tags, save_data, load_data, save_dataframe, load_dataframe
from sustainameal.ordering import sort_recipes_by_healthiness_score, sort_recipes_by_sustainability_score, \
...
class SustainaMeal:
    ...
    def _load_saved_data(self):
        if os.path.exists('/kaggle/input/embeddings/processed_recipes_df.pkl'):
            self.recipes_df = pd.read_pickle('/kaggle/input/embeddings/processed_recipes_df.pkl')
        else:
            raise FileNotFoundError("Processed recipes DataFrame not found.")

        if os.path.exists('/kaggle/input/embeddings/title_embeddings.pkl'):
            with open('/kaggle/input/embeddings/title_embeddings.pkl', 'rb') as f:
                self.title_embeddings = pickle.load(f)
        else:
            raise FileNotFoundError("Title embeddings not found.")

        with open('/kaggle/input/embeddings/vectorized.pkl', 'rb') as f:
    ...
    def _process_and_save_data(self, recipes_df):
        ...
        save_dataframe(self.recipes_df, '/kaggle/working/processed_recipes_df.pkl')
    ...
    def _initialize_system(self):
        ...
        if self.title_embeddings is None:
            ...
            save_data(self.title_embeddings, '/kaggle/working/title_embeddings.pkl')
        ...
        if self.nutrient_vectors_df is None:
            ...
            with open('/kaggle/working/vectorized.pkl', 'wb') as f:
    ...
    def find_similar_recipes(self, input_text, k, acceptable_tags, match_all_tags, check_sustainability=False, j=5):
        ...
        if similarity_score > 0.99 :
            ...
            centroid, common_tags, mean_who_score, mean_sustainability_score = calculate_centroids_and_find_common_tags(
                ...
                self.nutrients, self.vectorized, input_recipe=input_text, acceptable_tags=acceptable_tags)
